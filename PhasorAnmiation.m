...
for c = 1 : N
 ...
 %Voltage  
   ...
   plot(xc,yc,'m','lineWidth',1,LineStyle=':');  % Perimeter of circle
...
% Current
   ...
   xP = [12+t(c) 12+t(c)]; yP = [0 yPP(c)];% Vertical component of sin wave (red trcking line)
   plot(xP,yP,'k','LineWidth',3);
      
   plot(xcc,ycc,'y','lineWidth',2,LineStyle=':');  % Perimeter of circle
   
   xP = 12+t;  % Background sin wave (pink)
   plot(xP,yPP,'y','LineWidth',1); 
   
   xP = 12+t(1:c); yP = yPP(1:c); % Horizontal component of sin wave (green trcking line)
